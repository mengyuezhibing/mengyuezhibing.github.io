...
纯 HTML / CSS / Vanilla JS 实现的单页作品展示站，零依赖、无构建步骤，可直接托管在 GitHub Pages。
...
- 新增**左侧固定菜单栏**：贯穿竖线轨道 + 编号 + 名称，当前板块用金色高亮段标记，滚动自动跟随，与顶部导航双向同步，≥1200px 显示
...
3. **Canvas 背景**：粒子连线 + 鼠标磁力线，已处理 DPR 与 `prefers-reduced-motion`
...
> 作品封面推荐尺寸 **1920×1080**。你可以把它放进仓库 `assets/` 目录后写相对路径，例如 `cover: './assets/work-01.jpg'`。
